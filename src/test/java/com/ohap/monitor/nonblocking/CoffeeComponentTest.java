package com.ohap.monitor.nonblocking;

import com.ohap.monitor.repository.CoffeeComponent;
...
public class CoffeeComponentTest {
    ...
    public void 가격_조회_비동기_블록킹_호출_테스트(){
        ...
        int resultPrice = future.join();  //블록킹 1초
        log.info("최종 가격 전달 받음");

        future = coffeeComponent.getPriceAsync("latte");
        log.info("아직 최종 데이터를 전달 받지는 않았지만, 다른 작업 수행 가능");
        //resultPrice = future.join();  //블록킹 1초
        log.info("최종 가격 전달 받음");
        future = coffeeComponent.getPriceAsync("latte");
        log.info("아직 최종 데이터를 전달 받지는 않았지만, 다른 작업 수행 가능");
        //resultPrice = future.join();  //블록킹 1초
